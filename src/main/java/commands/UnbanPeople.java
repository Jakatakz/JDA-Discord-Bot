...
public class UnbanPeople extends ListenerAdapter
{
						...
						if (message.contains(a))
								{
									...
									event.getGuild().getController().unban(userIDForUnban).queue();
									//need to update users table with updated record
									try
									{
										String sqlUnban = "UPDATE users SET warnings = ? , banned = ? , bantimestamp = ? WHERE userid = ?";
	...
	{
		String sqlListAllMembersEver = "SELECT name FROM users;";
	...
	public String getUserIDForUnban(String a)
	{
		...
		String sqlUserIDForUnban = "SELECT userid FROM users WHERE name = ?";
		String userID = "";
		try
		{
